...
export default function Dashboard() {
  ...
  // State i ri per Mobile Menu (Responsivitet)
    ...
    <div className="flex flex-col md:flex-row h-screen bg-slate-950 text-slate-200 font-sans relative overflow-hidden">
      ...
      <aside className={`absolute md:relative z-50 top-0 left-0 h-full w-[280px] md:w-[320px] bg-slate-900/95 md:bg-slate-900/50 border-r border-slate-800/50 flex flex-col shadow-[25px_0_50px_rgba(0,0,0,0.4)] shrink-0 transition-transform duration-300 backdrop-blur-xl md:backdrop-blur-md ${isMobileMenuOpen ? 'translate-x-0' : '-translate-x-full md:translate-x-0'}`}>
        ...
        <div className="flex-1 overflow-y-auto p-4 md:p-6 space-y-6 md:space-y-8 no-scrollbar flex flex-col">
          <div className="flex items-center gap-3 md:gap-4 p-3 md:p-4 bg-slate-800/40 rounded-[20px] md:rounded-3xl border border-slate-700/50 shadow-inner">
            ...
            <div className="truncate">
              <p className="font-black text-[10px] md:text-[11px] text-white truncate uppercase tracking-tight">{user?.user_metadata?.display_name || "Lena"}</p>
              <p className="text-[8px] md:text-[9px] text-indigo-400 font-black uppercase truncate tracking-widest">{user.email}</p>
            </div>
          </div>

          <nav className="flex flex-col gap-2 md:gap-3">
            <button onClick={() => router.push('/generate')} className="w-full py-3 md:py-4 bg-emerald-600 text-white rounded-[14px] md:rounded-2xl font-black text-[9px] md:text-[10px] uppercase tracking-widest shadow-[0_10px_20px_rgba(5,150,105,0.3)] transition-all hover:-translate-y-1">✨ Krijo Postime</button>
            ...
            <button onClick={() => router.push('/vision')} className="w-full py-3 md:py-4 bg-gradient-to-r from-indigo-600 via-purple-600 to-pink-600 text-white rounded-[14px] md:rounded-2xl font-black text-[9px] md:text-[10px] uppercase tracking-[0.2em] shadow-[0_10px_20px_rgba(124,58,237,0.3)] border border-white/10 flex items-center justify-center gap-2 md:gap-3 group transition-all hover:-translate-y-1"><span className="text-xs md:text-sm group-hover:rotate-12 transition-transform">📸</span>Image to Text</button>
            <button onClick={() => router.push('/text-to-image')} className="w-full py-3 md:py-4 bg-gradient-to-r from-amber-500 to-orange-600 text-white rounded-[14px] md:rounded-2xl font-black text-[9px] md:text-[10px] uppercase tracking-[0.2em] shadow-[0_10px_20px_rgba(245,158,11,0.3)] border border-white/10 flex items-center justify-center gap-2 md:gap-3 group transition-all hover:-translate-y-1"><span className="text-xs md:text-sm group-hover:scale-125 transition-transform">🎨</span>Text to Image</button>
          </nav>
        </div>
        
        <div className="p-6 md:p-8 border-t border-slate-800/50 bg-slate-900/40 mt-auto">
          <button onClick={handleLogout} className="w-full py-3 md:py-4 bg-red-500/5 border border-red-500/10 rounded-[14px] md:rounded-2xl text-[9px] md:text-[10px] font-black text-red-500 uppercase tracking-[0.3em] hover:bg-red-500 hover:text-white transition-all italic flex items-center justify-center gap-2 cursor-pointer">Logout 👋</button>
        </div>
      </aside>
        ...
        <div className="max-w-5xl mx-auto space-y-8 md:space-y-12">
          ...
          <section className="grid grid-cols-1 lg:grid-cols-3 gap-6 md:gap-8">
            <div className="lg:col-span-2 bg-slate-900/50 border border-slate-800/50 p-6 md:p-10 rounded-[30px] md:rounded-[45px] shadow-2xl flex flex-col h-[300px] md:h-[380px] backdrop-blur-sm">
              ...
              <div className="flex-1 w-full pb-6 md:pb-10">
                <ResponsiveContainer width="100%" height="100%">
                    <BarChart data={chartData} margin={{ top: 0, right: 10, left: -20, bottom: 0 }} barSize={window.innerWidth < 768 ? 20 : 45}>
                        <XAxis dataKey="name" axisLine={false} tickLine={false} fontSize={window.innerWidth < 768 ? 9 : 11} fontWeight="900" tick={{fill: '#475569'}} dy={12} />
                        <Tooltip cursor={{fill: '#1e293b', radius: 15}} contentStyle={{backgroundColor: '#0f172a', border: 'none', borderRadius: '15px'}} />
                        <Bar dataKey="count" fill="#4f46e5" radius={[12, 12, 12, 12]} />
                    </BarChart>
                </ResponsiveContainer>
          ...
          <section className="bg-slate-900/50 border border-slate-800/50 p-4 sm:p-6 md:p-8 rounded-[30px] md:rounded-[40px] backdrop-blur-sm shadow-2xl overflow-hidden min-h-[500px] md:min-h-[650px] flex flex-col">
              <div className="flex justify-between items-center mb-4 md:mb-6 px-2 shrink-0">
                <h3 className="text-xs md:text-sm font-black uppercase tracking-[0.2em] md:tracking-[0.4em] text-slate-500">Marketing Calendar</h3>
              </div>
          ...
          <section className="pt-4 md:pt-6 pb-10 md:pb-20">
             ...
             <div className="bg-slate-900/50 rounded-[25px] md:rounded-[40px] border border-slate-800/50 shadow-2xl overflow-hidden backdrop-blur-sm w-full">
                <div className="overflow-x-auto w-full no-scrollbar">
                  <div className="min-w-[700px]">
                    <div className="grid grid-cols-5 p-4 md:p-7 bg-slate-900 border-b border-slate-800 font-black text-slate-500 uppercase text-[9px] md:text-[10px] tracking-widest text-center">
                      <div className="text-left pl-4 md:pl-6">Client</div>
                      <div>Scheduled</div>
                      <div>Vault</div>
                      <div>Export</div>
                      <div className="text-right pr-6 md:pr-12">Action</div>
                    </div>

                    <div className="divide-y divide-slate-800/50">
                      {recentPosts.map((post) => (
                        <div key={post.id} className="grid grid-cols-5 p-4 md:p-7 hover:bg-slate-800/40 transition-all items-center group text-center">
                          <div onClick={() => router.push(`/generations/${post.id}`)} className="font-black text-white text-sm md:text-lg text-left pl-4 md:pl-6 truncate cursor-pointer tracking-tight">{post.business_name}</div>
                          <div className="text-[10px] md:text-xs font-bold text-indigo-400">{post.start_date ? moment(post.start_date).format('DD MMM') : '-'}</div>
                          <div className="flex justify-center">
                            <button onClick={() => toggleFavorite(post.id, post.is_favorite)} className={`text-xl md:text-3xl transition-all ${post.is_favorite ? 'text-indigo-500' : 'text-slate-700'}`}>
                              {post.is_favorite ? '★' : '☆'}
                            </button>
                          </div>
                          <div className="flex justify-center">
                          ...
                          <div className="text-right pr-4 md:pr-8">
                              <button 
                                ...
                                className="bg-red-950/20 text-red-500 px-3 md:px-5 py-2 md:py-2.5 rounded-[12px] md:rounded-[18px] text-[8px] md:text-[10px] font-black uppercase md:opacity-0 group-hover:opacity-100 border border-red-900/30 hover:bg-red-500 hover:text-white transition-all cursor-pointer"
                              >
                                Delete ✕
                              </button>
                          </div>
      ...
      {selectedDayEvents && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 md:p-6">
          <div className="absolute inset-0 bg-slate-950/80 backdrop-blur-xl" onClick={() => setSelectedDayEvents(null)}></div>
          <div className="relative w-full max-w-2xl bg-slate-900 border border-slate-800 rounded-[30px] md:rounded-[45px] shadow-3xl overflow-hidden">
            <div className="p-6 md:p-8 border-b border-slate-800 flex justify-between items-center bg-slate-900/50">
              ...
              <button onClick={() => setSelectedDayEvents(null)} className="w-10 h-10 md:w-12 md:h-12 flex items-center justify-center rounded-full bg-slate-800 text-white font-black hover:bg-red-500 transition-all cursor-pointer">✕</button>
            </div>
            <div className="p-2 md:p-4 max-h-[300px] md:max-h-[400px] overflow-y-auto no-scrollbar">
              <table className="w-full text-left">
                <tbody className="divide-y divide-slate-800/50">
                  {selectedDayEvents.map((ev, idx) => (
                    <tr key={idx} className="group hover:bg-slate-800/30 flex flex-col sm:table-row">
                      <td className="p-3 md:p-4 font-black text-white italic text-sm md:text-base">{ev.business}</td>
                      <td className="p-3 md:p-4 text-[10px] md:text-xs font-bold text-slate-400">{ev.title}</td>
                      <td className="p-3 md:p-4 sm:text-right flex gap-2 sm:justify-end border-b sm:border-0 border-slate-800/50">
                        <PdfButton post={ev.fullData} />
                        <button onClick={() => router.push(`/generations/${ev.id}`)} className="bg-slate-800 text-white text-[8px] md:text-[9px] font-black uppercase px-3 md:px-4 py-1.5 md:py-2 rounded-full hover:bg-indigo-600 transition-all cursor-pointer">View</button>
                      </td>
                    </tr>
                  ))}
                </tbody>
          ...
          <div className="relative w-full max-w-md bg-slate-900 border border-red-500/20 rounded-[30px] md:rounded-[45px] shadow-[0_0_50px_rgba(239,68,68,0.15)] overflow-hidden animate-in fade-in zoom-in duration-200">
            <div className="p-6 md:p-10 text-center space-y-4 md:space-y-6">
              ...
              <div className="flex flex-col gap-2 md:gap-3 pt-4">
                <button 
                  onClick={confirmDelete}
                  className="w-full py-3 md:py-4 bg-red-600 text-white rounded-[14px] md:rounded-2xl font-black text-[9px] md:text-[10px] uppercase tracking-[0.2em] shadow-[0_10px_20px_rgba(220,38,38,0.3)] hover:bg-red-500 transition-all cursor-pointer"
                >
                  Po, Fshije Përgjithmonë
                </button>
                <button 
                  onClick={() => setDeleteTarget(null)}
                  className="w-full py-3 md:py-4 bg-slate-800 text-slate-400 border border-slate-700 rounded-[14px] md:rounded-2xl font-black text-[9px] md:text-[10px] uppercase tracking-[0.2em] hover:text-white transition-all cursor-pointer"
                >
                  Anulo
                </button>
              </div>
      ...
      <style jsx global>{`
        ...
        /* Përshtatje shtesë për tabelën e kalendarit në pajisje mobile */
        @media (max-width: 768px) {
          .custom-calendar-style .rbc-toolbar { flex-direction: column; gap: 10px; margin-bottom: 15px; }
          .custom-calendar-style .rbc-toolbar-label { margin-top: 5px; margin-bottom: 5px; }
          .custom-calendar-style .rbc-header { padding: 8px 4px; font-size: 8px; }
          .custom-calendar-style .rbc-event { font-size: 8px; padding: 2px 4px; }
        }

        .custom-calendar-style { color: #94a3b8; font-family: inherit; height: 100% !important; min-height: 500px; }
        .custom-calendar-style .rbc-month-view { border: 1px solid #1e293b; background: #0f172a; border-radius: 20px; overflow: hidden; }
        @media (min-width: 768px) {
          .custom-calendar-style .rbc-month-view { border-radius: 30px; }
        }
        .custom-calendar-style .rbc-header { padding: 15px; font-weight: 900; text-transform: uppercase; font-size: 10px; color: #64748b; border-bottom: 2px solid #1e293b; }
        .custom-calendar-style .rbc-day-bg { border-left: 1px solid #1e293b; }
        .custom-calendar-style .rbc-today { background: rgba(79, 70, 229, 0.1); }
